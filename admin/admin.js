...
function normalizeR2Data(raw) {
    ...
    // 如果已经是标准的具有有效 bucket1..bucket11 且有实际用量的结构
...
function renderR2Dashboard(rawData, litCount) {
    ...
    if (b10StatusBadge) {
        ...
        b10StatusBadge.textContent = b10.status_level === 'locked' ? '安全阀锁死' : (b10.status_level === 'warning' ? `${b10.used_ratio}% 预警` : (b10.status_level === 'critical' ? '熔断' : '就绪待命'));
    }
    ...
    // 12. Bucket 11 环形仪表盘与指标
    const b11GaugeProgress = document.getElementById('b11-gauge-progress');
    const b11GaugePct = document.getElementById('b11-gauge-pct');
    const b11GaugeVal = document.getElementById('b11-gauge-val');
    const b11StatusBadge = document.getElementById('b11-status-badge');
    const b11StatSongs = document.getElementById('b11-stat-songs');
    const b11StatSize = document.getElementById('b11-stat-size');

    if (b11GaugeProgress) {
        const offset11 = c * (1 - Math.min(100, Math.max(0, b11.used_ratio || 0)) / 100);
        b11GaugeProgress.style.strokeDasharray = `${c}`;
        b11GaugeProgress.style.strokeDashoffset = `${offset11.toFixed(2)}`;
        b11GaugeProgress.setAttribute('class', `gauge-progress stroke-${b11.status_level || 'healthy'}`);
    }
    if (b11GaugePct) b11GaugePct.textContent = `${(b11.used_ratio || 0.0).toFixed(1)}%`;
    if (b11GaugeVal) {
        if (b11.used_gb > 0) {
            b11GaugeVal.textContent = `${b11.used_gb} GB`;
        } else if (b11.used_mb > 0) {
            b11GaugeVal.textContent = `${b11.used_mb.toFixed(0)} MB`;
        } else {
            b11GaugeVal.textContent = '0 B';
        }
    }
    if (b11StatusBadge) {
        b11StatusBadge.className = `bucket-badge badge-${b11.status_level || 'healthy'}`;
        b11StatusBadge.textContent = b11.status_level === 'locked' ? '安全阀锁死' : (b11.status_level === 'warning' ? `${b11.used_ratio}% 预警` : (b11.status_level === 'critical' ? '熔断' : '主力写入'));
    }
    if (b11StatSongs) b11StatSongs.textContent = (b11.songs_count || 0).toLocaleString();
    if (b11StatSize) {
        if (b11.used_gb > 0) {
            b11StatSize.textContent = `${b11.used_gb} GB`;
        } else if (b11.used_mb > 0) {
            b11StatSize.textContent = `${b11.used_mb.toFixed(1)} MB`;
        } else {
            b11StatSize.textContent = '0.0 MB';
        }
    }

    // 13. Bucket 12 环形仪表盘与指标
    const b12GaugeProgress = document.getElementById('b12-gauge-progress');
    const b12GaugePct = document.getElementById('b12-gauge-pct');
    const b12GaugeVal = document.getElementById('b12-gauge-val');
    const b12StatusBadge = document.getElementById('b12-status-badge');
    const b12StatSongs = document.getElementById('b12-stat-songs');
    const b12StatSize = document.getElementById('b12-stat-size');

    if (b12GaugeProgress) {
        const offset12 = c * (1 - Math.min(100, Math.max(0, b12.used_ratio || 0)) / 100);
        b12GaugeProgress.style.strokeDasharray = `${c}`;
        b12GaugeProgress.style.strokeDashoffset = `${offset12.toFixed(2)}`;
        b12GaugeProgress.setAttribute('class', `gauge-progress stroke-${b12.status_level || 'healthy'}`);
    }
    if (b12GaugePct) b12GaugePct.textContent = `${(b12.used_ratio || 0.0).toFixed(1)}%`;
    if (b12GaugeVal) {
        if (b12.used_gb > 0) {
            b12GaugeVal.textContent = `${b12.used_gb} GB`;
        } else if (b12.used_mb > 0) {
            b12GaugeVal.textContent = `${b12.used_mb.toFixed(0)} MB`;
        } else {
            b12GaugeVal.textContent = '0 B';
        }
    }
    if (b12StatusBadge) {
        b12StatusBadge.className = `bucket-badge badge-${b12.status_level || 'healthy'}`;
        b12StatusBadge.textContent = b12.status_level === 'locked' ? '安全阀锁死' : (b12.status_level === 'warning' ? `${b12.used_ratio}% 预警` : (b12.status_level === 'critical' ? '熔断' : (b12.songs_count > 0 ? '主力写入' : '就绪待命')));
    }
    if (b12StatSongs) b12StatSongs.textContent = (b12.songs_count || 0).toLocaleString();
    if (b12StatSize) {
        if (b12.used_gb > 0) {
            b12StatSize.textContent = `${b12.used_gb} GB`;
        } else if (b12.used_mb > 0) {
